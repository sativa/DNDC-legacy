                              ...
                              float *day_ch4_CO2, float *day_ch4_1, float *day_ch4_2, float *SO4, float day_root_resp)
{
    ...
    {
        //clay effect----------------------------------------------
        Coef1 = min(300.0, 122.88 * (float)pow(FFclay, (float)-0.7414));//300 ???		 
        ...
        //methanogen activity as a function of time 
         ...
         else
         {
             ...
             {
                F_methanogen[l] = (50.0 * exp(0.047*(float)FloDays) * (1.0 + pow(F_clay,(float)0.5))-0.3) * Tttoo[l];
             }
             else
             {	
                 if(WaterControl==1) 
                     F_methanogen[l] = (5.0 * exp(0.047*(float)FloDays) * (1.0 + pow(F_clay,(float)0.5))-0.3) * Tttoo[l] ;
                 else
                 {
                     if(flood_num==1)
                        F_methanogen[l] = (5.0 * exp(0.047*(float)FloDays) * (1.0 + pow(F_clay,(float)0.5))-0.3) * Tttoo[l] ;//0.1;
                     else
                        F_methanogen[l] = 50.0 * (0.2259 * log((float)FloDays) + 0.113) * (1.0 + pow(F_clay,(float)0.5)) * (1.0 / (float)(flood_num-1)) * Tttoo[l];//50.0
                 }		
             }
         }
     ...
     if(GN==q)//cropping land soil
     {
        if (TotalRootWT>0.0)
        {
             float bq = (float)pow((float)0.5, (float)l);
             float fPGI = 1.0;
             if(u_PGI>0.5) 
             {
                 fPGI = 1.0 - u_PGI * 0.5;
           ...
           if(N_stress[1]<1.0) InputRoot_DOC[l] *= ((float)pow(clay[l], (float)0.25) / N_stress[1]);
     ...
     if(temp[l]<=0.0) 
         ...
         ch4_max = ch4_max_30 * pow(Q10CH4, (float)( (temp[l] - 0.0)/10.0 ) ); //!
     else 
