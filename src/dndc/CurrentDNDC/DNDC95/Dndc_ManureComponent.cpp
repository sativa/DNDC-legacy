...
void class_model::FarmFacilityCompost()
{
	q=10;
		...
		{	
			...
			if(soc[GN]>0.0)
			{
				...
				MWM->compost_BGC(
					...
					R_Wt, vascularity, wiltpt, Odoc, &day_frostDOC, &yr_frostDOC, &winertC, inert_C, fert_flag, root_doc, //180
					precipitation, manuFlag, irri_flag, FreezeDOC, day_O2, RFM, SoilDepth, rcnh2, Eh1, sop, //190
					OrgP, LabP, ComP, AdsP, LateralInflux, drain_flag, day_leach_water, CompostCover);	
...
/*			if(MData.m_LagoonCover==1)//air open
			...
			if(soc[GN]>0.0)
			{
				...
				MWM->lagoon_BGC(FN, GN, q, jday, rcnrvl, rcnrl, rcnrr, rcnh, rcnb, 
					...
					R_Wt, vascularity, Odoc, &winertC, inert_C, fert_flag, root_doc, //177
