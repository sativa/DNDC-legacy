...
void class_manure::HouseManure_BGC(int FN, int GN, 
		...
		float *SO4, float sslope, float day_root_resp)
{
	int i;
...
*/		
	...
	else //indoor feedlot
	{
		...
		else //-1 indoor climate file data
		{
			...
			m_wind_speed = MData.mm_VentRate * 0.1;//MData.indoor_airflow * 0.1;
			//m_PAR = PAR;
		}
...
*/
	...
	if(MData.mm_GroundSurface[1]!=3)
	{
		DNDC->N2O_balloon(GN, till_fact, sts, h, jday, *flood_flag, FloDays, rainfall,
			...
			nh4, no3, no2, no, n2, Odoc, n2o, doc_w, nh4_w, no3_w, no2_w, no_w, n2o_w, n2_w,
			...
			manuFlag, irri_flag, FreezeDOC, nh3, Eh1, urea, 0.0, 0, 0);
	}
