...
void class_model::Main_Feedlot(int year, int scale, FILE* fout2, char *F_CLIMD,
							   ...
							   float till_fact, float *Tttoo, FILE* ftf, float ha, int Batch, 
							   int livestock, int q, char *PassL1, int FarmField, char *Pass)
{
	...
	for(fln=1; fln<=m_FeedLots; fln++) ////////////////////////////////////////////////////////////
	{
		...
		else if(MData.InputFormat[FN]==3)//use a nutrition model
		{
			...
			{
				//run sow model to generate "swiner/Sow_out_file.csv"
				int KM=0;
				
				KM=system(".\\R-3.0.1\\bin\\Rscript.exe .\\swiner\\sow_model_run.R");				
				if(KM==1)
				{
					AfxMessageBox("The model will close. You will need to reopen Manure-DNDC and then just click RUN");
					exit(0);
				}

				if(KM==0)
				{
					AfxMessageBox("You have created Sow_input_file.csv");
					
				}
			}
			else if(MData.NutritionModel[FN]==4)
			{
				//run growth model to generate "swiner/Growth_input_file.csv"
				int KM=0;
				KM=system(".\\R-3.0.1\\bin\\Rscript.exe .\\swiner\\growth_model_run.R");				
				
				//KM=system("C:\\ManureDNDC\\R-3.0.1\\bin\\Rscript.exe .\\swiner\\growth_model_run.R");				
				...
				if(KM==0)
				{
					AfxMessageBox("You have created Growth_input_file.csv");
		...
		if(MData.InputFormat[1]==3) 
		{
			if(MData.NutritionModel[1]==5)
				sprintf(F_LV1, "%s", "C:\\ManureDNDC\\swiner\\sow_model_out.csv");

			if(MData.NutritionModel[1]==4)
				sprintf(F_LV1, "%s", "C:\\ManureDNDC\\swiner\\growth_model_out.csv");
		...
		if (scale == 0)
		{
			...
			for(jday=1; jday<=365; jday++)
			{
				...
				MUM->ManureFeedlot(
					...
					&pen_soil_wfps, SO4, sslope, day_root_resp);//, &House_to_compost_C, &House_to_compost_N, &House_to_compost_H2O);
