...
*/
    ...
    int NoRainFlag;
    float StormIndex, horricane_event, hail_event, day_SoilWater;
    ...
    float hrDemand, FloodWater_N, pen_soil_wfps;
...
#ifdef DEGUI
#else	
    void write_MonteCarlo(int sample, CString county_ID1, char* cropping_system);
    void write_BatchSummary(int sample, CString county_ID1, char* cropping_system);
#endif
/*	void write_region_summary(int SoilYear, float ha, int landuse, int province, int county,
   ...
   //void read_in_climmusle();
    ...
    void hour_loop(float hrPE0, float hrPT0,float* DayFlux,float* dby_flux, int DroughtID, int YieldToday, 
        int drc, int scale, float ha, int livestock);
    
    void hourly_transp_and_water_stress(int hr, float hrPT0, int DroughtID, int YieldToday, int drc);
    float crop_transpiration(float DayPT0, int irri_close);
    ...
    void crop_N_demand(int ccwc, float DayPT2);
...
//	void crop_leaf_litter_deposition(float AddCN, float AddC);
    void crop_N_uptake();
    void crop_water_demand(float DayPET, float* hrPT0, float wreq_weed );
    void crop_root_CM1();
...
*/
    ...
    void dndc_decomposition(float* CRB1, float* CRB2, float* newh, float* w_no2, 
        ...
        int *crop, float *yr_fixn, float rainfall, float *soil_ph, float *Odoc, float *winert_C,
    ...
    void Main_Feedlot(int year, int scale, FILE* fout2, char *F_CLIMD,
                        ...
                        float till_fact, float *Tttoo, FILE* ftf,
                        float ha, int Batch, int livestock, int q, char *PassL1, int FarmField, char *Pass);
    void Main_Treatment(int year, int scale, char *F_CLIMD, 
...
public:
  ...
  // public member functions
    ...
    void ManureFeedlot(
        ...
        float *day_inManureN, float *Odoc, float *day_frostDOC, float *yr_frostDOC, float *winertC, 
        ...
        float* pen_soil_wfps, float *SO4, float sslope, float day_root_resp);//, float *House_to_compost_C, float *House_to_compost_N, float *House_to_compost_H2O);
    ...
    void ManureTreatmentBGC(
        ...
        int CompostCover, float LagoonCapacity);//, float *House_to_compost_C, float *House_to_compost_N, float *House_to_compost_H2O);
    ...
    void HouseManure_BGC(int FN, int GN, 
        ...
        float *day_ch4_2, float day_leach_water, float* pen_soil_wfps, float *SO4, float sslope, float day_root_resp);
    ...
    void StorageManureRemoval(int FN, int GN, int q, float rcnrvl, float rcnrl, float rcnrr, float rcnb, float rcnh, float rcnm,
                                 ...
                                 FILE *ftf, float *day_farm_to_field_N, float *day_farm_to_field_C, 
