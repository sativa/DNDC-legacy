...
*/
        ...
        else
        {
            ...
            {
                float Assimilation = 1.0 - (float)pow((float)MData.lagoon_days,(float)1.1) / 365.0;
...

{
    ...
    //if ( l > (tilq + 3) ) f_till_fact = 1.0;
    if (l>=tilq) f_till_fact = 1.0;// + 3
    else  f_till_fact = (float)pow(till_fact, (float)2.0);

    // humads decomposition */
    if(l>q) clay[l] = 0.01;
    float Fclay = 0.5 * pow(clay[l], (float)-0.471);
        ...
        float *doc)
{
    ...
    //if(st[l]<12)
        Fclay = 0.1793 * pow(clay[l], (float)-0.471) * 20.0;
   ...
   int l;
    ...
    for (l=1; l<=q; l++)
    {
        if(NH4HCO3[l] > 0.0000001)
        {
            ...
            Fsf = 6.0 * dFF * (float)pow((float)10.0, Fsf);
...
#ifdef MANURE
        if(MData.m_Flash[l-q]>=1) ddf =  0.01 * (float)pow(soil_T, (float)0.9) + 0.2;
        else 
        {
            if(soil_T>1.0) ddf =  0.05 * (float)pow(soil_T, (float)1.0);
...
*/
        ...
        if(ddf>0.0)
        {
            ...
            //if(flood_flag==0)
            {
                ...
                hydrogen = (float)pow((float)10.0, -sph[l]);//mol H/L
...
void class_model::nh43_equil(int l, float *temp, float *sph, float *nh4, float *nh3, int q, float *h, float *day_wfps, int flood_flag)
{
    ...
    hydrogen = (double)pow((float)10.0, -sph[l]);//mol/L
...
{
    float EarthWormActivity = (float)pow((float)0.0001, (float)l);//0.01
...
void class_model::clay_adsorption(int l, float RFM, float *nh4, float *clay_nh4, float base_clay_N, float max_clay_N, int q, float *clay)////////N_adsorption
{
    ...
    lbcn = base_clay_N * (float)pow((float)0.5, (float)l);
    lmcn = max_clay_N * (float)pow((float)0.5, (float)l);
...
void class_model::NO3_leaching()//N_leaching
{
    ...
    fCLAY= 0.1 * (float)pow(clay[1],(float)-2.5);//-0.4907);//1.0, 0.8, 0.5
...
void class_model::Chemodenitrification(int l, float *temp)
{
    ...
    chem_no = f3 * (float)pow(day_soil_an_no, (float)1.5);
