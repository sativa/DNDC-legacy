...
{
    float KC=0, KN;
    float u_no3=0, u_no2=0, u_no=0, u_n2o=0, U=0, DBG=0, DB=0, Nw=0;
    float d_no3=0, d_no2=0, d_no2_kk=0, d_no=0, d_n2o=0;
    float dn_co2=0, d_co2=0, bypass_f=0;
    float Fde=1.0, Fph=1.0, Fmn=1.0, Fst=1.0, Fsm=1.0, Ftm=1.0;

    int   T=0, l=0, rain_hrs=0;
    float assim_N=0, air_poro_max=0;
    float TE[MAXSOILLAYER], phk1[MAXSOILLAYER], phk2[MAXSOILLAYER], phk3[MAXSOILLAYER];
    float cck=0, n3k=0, n2k=0, nok=0, n2o_k=0, DBD=0, da_no3=0;
    float da_no2=0, da_no2_kk=0, da_no=0;
    float da_n2o=0, DC=0, day_leach_DOC=0, PORO=0;
    float aere=0.0;
    float lost_DOC=0, lost_no3=0;
    float leach_factor;
    float fact_t=0, fact_m=0, nno=0;
    float wwzz=0, ae_n2o=0, ae_no=0;
    float micro_growth=0, micro_death=0, micro_delta=0, k=0, AA=0;
    int flood_day=0;
    float EEE=0, RRR=0, nitrifier[MAXSOILLAYER], denitrifier[MAXSOILLAYER]; 
    int BL=0, EL=0;
    ...
    for ( T = 1; T <= 24; T++ )
    {
        for ( l = BL; l <= EL; l++ )
        {
            ...
            if(Eh[l]>=460.0) //Threshold Eh for denitrification
                anvf[l] = (1.0 - Eh[l]/800.0) * 0.1;
            else 
                anvf[l] = 0.99;
   ...
   // Allocate NH4+, NO3-, NO2-, NO, N2O, and DOC to anaerobic balloon 
    if(T==1) 
    {
       //if (temp[l]>9.0) TE[l] = powf(2.0, ( temp[l] - 22.5) / 10.0);
       ...
       if(sph[l]<=7.0||manuFlag==1)
        ...
        phk3[l] = 5.0 * powf(10.0,-8) * (float)exp(2.5538 * sph[l]); 
...
/*	if(total_ice>0.0) 
    ...
    if(denitrifier[l]>0.0)
    {
        ...
        else DC = 0.05 * (U / YMC + MC) * denitrifier[l];
    ...
    if(nitrifier[l]>0.0)
    {
        fact_t = powf(3.503,(60.0-temp[l])/(60.0-34.22))
        ...
        {
            //wwzz = 0.001;
            wwzz = 5.0 * k * nitrifier[l] * (1.0 - powf(clay[l], 0.5)) * day_wfps[1] * sph[l] * till_fact * LFactor;//5.0; 10.0
...
/*		double wd1, Kw, V_water, increase_H, hydrogen, mol_nh4, hydroxide;
        ...
        hydrogen = powf(10.0, -sph[l]);//mol H/L
...
*/
        ...
        if(l<=q)
        {
            if(drain_flag==0)
            {
                if(LandUse==1||LandUse==2)
                {
                    if(flood_flag==1&&l==1)
                    {
                        EEE = 10.0 * powf(water[T][l]/(h[l]*sts[l]), 5.0);//0.01
                    }
                    else
                        EEE = 0.05 * powf(water[T][l]/(h[l]*sts[l]), 5.0);
                }
                else 
                {
                    EEE = 0.2 * powf(water[T][l]/(h[l]*sts[l]), 5.0);//0.2
                }
            }
            else 
                EEE = 0.1;
 ...
 /*if(flood_end==1&&flood_flag<=0)
 {
  ...
  if (flood_flag==0)
  {
    ...
    {
         pno = peat*0.5*(f_litter * ((.0006 + .0013 * AD) + (.013 - .005 * AD) * PA * powf(2.0, tt / 20.0)) * PORO);//Lena
         pn2o = peat*PA * (f_litter * (30.0 * (.0006 + .0013 * AD) + (.013 - .005 * AD) * powf(2.0, tt / 20.0) * PORO)); //*****
         pn2 = peat*f_litter * (0.017 + (.025 - .0013 * AD) * PA * powf(2.0, tt / 20.0) * PORO);
    }
  ...
  else //flood_flag = 1
  {
    ...
    if (l <= 10 )
    {
       ...
       ActTemp = (-.1687 * powf((0.1 * tt), 3.0) + 1.167 * 
                powf((0.1 * tt), 2.0) - 2.0303 * (0.1 * tt) + 1.042);
