								 ...
								 float *OrgP, float *LabP, float *ComP, FILE* fFeedOut)// float *House_to_compost_C, float *House_to_compost_N, float *House_to_compost_H2O)
{
	...
	if(CLD==0)
	{
		...
		for(i=1; i<=22; i++) 
		{
			fprintf(fFeedOut, "%f ", ToCompost[i]);
		}
		fprintf(fFeedOut, "\n");

		for(i=1; i<=22; i++) 
		{
			fprintf(fFeedOut, "%f ", ToLagoon[i]);
		}
		fprintf(fFeedOut, "\n");

		for(i=1; i<=22; i++) 
		{
			fprintf(fFeedOut, "%f ", ToDigester[i]);
		}
		fprintf(fFeedOut, "\n");

		for(i=1; i<=22; i++) 
		{
			fprintf(fFeedOut, "%f ", ToField[i]);
		}
		fprintf(fFeedOut, "\n");
...
*/
		...
		else
		{
			int i;
			fprintf(fFeedOut, "%d\n", jday);
			for(i=1; i<=22; i++) fprintf(fFeedOut, "%f ", 0.0);
			fprintf(fFeedOut, "\n");
			for(i=1; i<=22; i++) fprintf(fFeedOut, "%f ", 0.0);
			fprintf(fFeedOut, "\n");
			for(i=1; i<=22; i++) fprintf(fFeedOut, "%f ", 0.0);
...
void class_manure::StorageManureRemoval(int FN, int GN, int q, float rcnrvl, float rcnrl, float rcnrr, float rcnb, float rcnh, float rcnm,
								 ...
								 FILE *ftf, float *day_farm_to_field_N, float *day_farm_to_field_C, float *day_farm_to_field_H2O,
...
void class_manure::EntericGasProduction(int FN, float air_temp)
{
	...
	//for(i=1;i<=MData.AnimalTypes[FC];i++)
	{
		...
		if(MData.mm_AnimalType[FC]==1)// milking cows
		{
			...
			MData.day_enteric_ch4[FC] = GE * Ym / 55.65 * 12.0 / 16.0 * MData.mm_AnimalHeads[FC];// * (float)pow(Ftemp, 0.8); //daily enteric CH4 production, kg C/day per farm
