...
int SaveSoilParas(int id)//1
{
    ...
    char fname[255];
    FILE *fpo;
    sprintf(fname,"%s\\soil-%d", INPUTS,(int)fabs((float)id));
    fpo=fopen(fname,"w");
    if(fpo==NULL) note(1, fname);
    ...
    for (i=1; i<=Q; i++)
    {
        tt[i] = AveYrT + (Soil_T - AveYrT)/ (float)pow(i*H*100, (float)0.5);
...
for(int MFFC=1; MFFC<=FarmCrops; MFFC++)
{
    ...
    for (i = 1; i<=n;i++) 
    {
        ...
        fprintf(fp,"%d %f %f %d %d\n", day, ManuAmount[i], ManuCN[i], ManuType[i], ManuMethod[i]);
    }
    fclose (fp);
