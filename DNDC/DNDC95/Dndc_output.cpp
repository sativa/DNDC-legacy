...
int class_model::write_out_days(FILE *fs, float DayPET)
{
    ...
    //int ww=(int)((float)q/5.0);
     ...
     if (jday==1)
     {
        ...
        fprintf(fs, ",,,,Soil_temperature,,,,,,,Soil_moisture(wfps),,,,,,,Soil_oxygen(kg O2/layer),,,,,,Soil_Eh(mv),,,,,,WT(m), Ice(wfps),,,,,,Ice,Snowpack,SoilWater,DeepWater,Soil_pH,,,,,,SurfaceWaterT\n");
        ...
        fprintf(fs, "%7.1f,%7.1f,%7.1f,%7.1f,%7.1f,%7.1f,", O2_old[LL0], O2_old[LL1], O2_old[LL2], O2_old[LL3], O2_old[LL4], O2_old[LL5]);
...
int class_model::write_out_dayc(FILE *fc )
{
    ...
    fprintf(fc, "%7.2f,%7.2f,%7.2f,%7.2f,%7.2f,%7.2f,%7.2f,%7.2f,%7.2f,", 
        Psn, sum_shoot_CO2, sum_stem_CO2, day_RootCO2, soil_hete_CO2, dPlantBiomass, EcoCO2, NEE, stub1+stub2+stub3);
...
int class_model::write_out_dayg( FILE *fg )
{
    ...
    for(int i=1; i<=5; i++)
    {
        dung_C += day_graze_dung_C[i];
        dung_N += day_graze_dung_N[i];
        urine_C += day_graze_urine_C[i];
        urine_N += day_graze_urine_N[i];
        dung_P += day_graze_dung_P[i];
    }
...
int class_model::write_out_dayp(FILE *fcrop)
{
    ...
    if (jday==1)
    {
        ...
        for(i=1;i<=CropNumbers;i++) fprintf(fcrop, ",Crop %d,,,,,,,,,,,,,,,,,,,,,,,,,,,", i);
        fprintf(fcrop, "\n");		
...
if(m_unit==0)//Metric system
{
    ...
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12d ", i);
    fprintf(fout, "\n");

    fprintf(fout, "  Crop name			");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%-12s ", crop_name[i]);
    fprintf(fout, "\n");			
        
    fprintf(fout, "  Planting date		");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12d ", jdseed[i]);
    fprintf(fout, "\n");

    fprintf(fout, "  Growing days		");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12d ", grow_days[i]);
    fprintf(fout, "\n");
    
    fprintf(fout, "  Growing season TDD	");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.0f ", TDD[i]);
    fprintf(fout, "\n");

    fprintf(fout, "  Water demand (mm)	");	
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", crop_PT[i]*1000.0);
    fprintf(fout, "\n");

    fprintf(fout, "  Water uptake (mm)	");	
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", crop_trans[i]*1000.0);
    fprintf(fout, "\n");
    
    fprintf(fout, "  Water stress		");	
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", crop_water_stress[i]/G_days[i]);
    fprintf(fout, "\n");
                            
    fprintf(fout, "  Crop N demand		");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", maxmn[i]);// yr_need_N[i], demandn[i]
    fprintf(fout, "\n");
    
    fprintf(fout, "  Crop N from soil	");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", yr_uptaken[i]);
    fprintf(fout, "\n");
    
    fprintf(fout, "  Crop N from air NH3	");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", crop_NH3_absorp[i]);
    fprintf(fout, "\n");
    
    fprintf(fout, "  Crop N fixation	");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", yr_fixn[i]);
    fprintf(fout, "\n");
    
    fprintf(fout, "  Nitrogen stress	");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", crop_N_stress[i]/G_days[i]);
    fprintf(fout, "\n\n");
    
    fprintf(fout, "  Annual crop biomass production:\n");
    fprintf(fout, "  Crop N (kg N/ha)	");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", yr_plantn[i]);
    fprintf(fout, "\n");
    
    fprintf(fout, "  Crop C (kg C/ha)	");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", Yr_PlantBiomass[i]);
    fprintf(fout, "\n");
    ...
    fprintf(fout, "  -- Leaf C		");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", YrG_leaf[i]);
    fprintf(fout, "\n");

    fprintf(fout, "  -- stem C		");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", YrG_stem[i]);
    fprintf(fout, "\n");
                        
    fprintf(fout, "  -- Root C		");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", YrG_root[i]);
    fprintf(fout, "\n\n");
    ...
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%8.0f", yr_CutGrain[i]);
    fprintf(fout, "\n");

    fprintf(fout, "  Leaf cut (kg C/ha)		");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%8.0f", yr_CutLeaf[i]);
    fprintf(fout, "\n");

    fprintf(fout, "  Stem cut (kg C/ha)		");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%8.0f", yr_CutStem[i]);
    fprintf(fout, "\n");

    fprintf(fout, "  Root cut (kg C/ha)		");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%8.0f", yr_CutRoot[i]);
    fprintf(fout, "\n");
...
else//English system
{
    ...
    fprintf(fout, "  Crop N demand		");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", maxmn[i]*w3);// yr_need_N[i], demandn[i]
    fprintf(fout, "\n");
    
    fprintf(fout, "  Crop N from soil	");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", yr_uptaken[i]*w3);
    fprintf(fout, "\n");
    
    fprintf(fout, "  Crop N from air NH3	");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", crop_NH3_absorp[i]*w3);
    fprintf(fout, "\n");
    
    fprintf(fout, "  Crop N fixation	");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", yr_fixn[i]*w3);
    fprintf(fout, "\n");
    ...
    fprintf(fout, "  Crop N (kg N/ha)	");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", yr_plantn[i]*w3);
    fprintf(fout, "\n");
    
    fprintf(fout, "  Crop C (kg C/ha)	");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", Yr_PlantBiomass[i]*w3);
    fprintf(fout, "\n");
    
    fprintf(fout, "  -- Grain C		");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", YrG_grain[i]*w3);
    fprintf(fout, "\n");
                                                
    fprintf(fout, "  -- Leaf C	");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", (YrG_leaf[i])*w3);
    fprintf(fout, "\n");	
    
    fprintf(fout, "  -- stem C	");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", (YrG_stem[i])*w3);
    fprintf(fout, "\n");
                        
    fprintf(fout, "  -- Root C		");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%12.2f ", YrG_root[i]*w3);
    fprintf(fout, "\n\n");
    ...
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%8.0f", yr_CutGrain[i]*w3);
    fprintf(fout, "\n");

    fprintf(fout, "  Leaf cut (lb C/ac)		");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%8.0f", yr_CutLeaf[i]*w3);
    fprintf(fout, "\n");

    fprintf(fout, "  Stem cut (lb C/ac)		");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%8.0f", yr_CutStem[i]*w3);
    fprintf(fout, "\n");

    fprintf(fout, "  Root cut (lb C/ac)		");
    for(i=1;i<=CropNumbers;i++) fprintf(fout, "%8.0f", yr_CutRoot[i]*w3);
    fprintf(fout, "\n");
...
//#endif		
    ...
    for(int i=1; i<=3; i++)
        fprintf(fout, "%.2f	Crop_grain1 (kg C/ha):	\n", YrG_grain[i]);//+ yr_CutGrain[i]+yr_GrazeGrain[i]);
    
    for(int i=1; i<=3; i++)	
        fprintf(fout, "%.2f	Crop_shoot1 (kg C/ha):	\n", YrG_leaf[i]+YrG_stem[i]);// + yr_CutLeaf[i]+yr_CutStem[i]+yr_GrazeShoot[i]);
    
    for(int i=1; i<=3; i++)
        fprintf(fout, "%.2f	Crop_root1 (kg C/ha):	\n", YrG_root[i]);// + yr_CutRoot[i]+yr_GrazeRoot[i]);
...
#ifdef DEGUI
#else	
void class_model::write_MonteCarlo(int sample, CString county_ID1, char* cropping_system)
{
    float dSOC;
...
void class_model::write_BatchSummary(int sample, CString county_ID1, char* PassC1)
{
    float dSOC;
    FILE  *fp;
    char  IE[120];
    ...
    //if(Option>100)
    {
        ...
        fprintf(mc, "%10.2f, %10.2f, %10.2f, %10.2f, %10.2f, %10.2f, %10.2f, %10.2f, %10.2f, %10.2f, %10.2f, %10.2f, %-40s\n",
                 MC_grainC,MC_shootC,MC_rootC,MC_CutPlantC,MC_GrazePlantC, dSOC, yr_ch4, yr_dnn2o, yr_dnno, yr_dnn2, yr_soil_nh3, yr_leach_NO3+yr_leach_urea, county_ID1);
    }
/*	else
    {
        float SOC, Clay, pH, BD;
