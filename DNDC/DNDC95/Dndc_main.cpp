...
//#endif
#endif
    ///////////// End of single farm facility simulation
*/	
    ...
    if(scale==0)
    {
        if(Batch==1)
        {
            ...
            fscanf(region, "%s", BatchPass);		
            fclose (region);*/
#ifdef MANURE
            sprintf(PassC1, "%s\\CropSystem\\%d", BatchPass, FarmField);
            mkdir(PassC1);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //if(FarmField==1)
    {
        ...
        Main_Feedlot(year, scale, fout2, F_CLIMD, w_no2, w_nh3, w_no3, w_nh4, wurea, day_clayn, wpool_no,
                     tilq, fCB, fNB, w_n2, litco22, litco23, till_fact, ttoo, ftf, ha, Batch, livestock, 
                     q, PassL1, FarmField, Pass);
...
//{
    ...
    {
        sprintf(FCT20, "%s\\FarmCrop_%d", OUTPUT, FarmField);//pass for reading input
        sprintf(FCT40, "%s\\FarmCrop_%d", INPUTS, FarmField);//pass for reading input
    } 
#else
...
///////////////////////////////////////////////////////
    ...
    float DayPE=0.0, DayPT0=0.0, DayPT2=0.0;
    int ccwc;
    ...
    if(DayPT0>0.0)
    {
        DayPT2 = crop_transpiration(DayPT0, irri_close);
    }
    else
    {
        DayPT2 = 0.0;		
    ...
    else DayWaterStress = DayPT2 / DayPT0;
    //if(DayPT0<=0.0) DayWaterStress = 1.0;
    ...
    soil_hete_CO2 = day_co2;	
    Psn = dPlantBiomass + sum_shoot_CO2 + sum_stem_CO2 + day_RootCO2;
    EcoCO2 = soil_hete_CO2 + sum_shoot_CO2 + sum_stem_CO2 + day_RootCO2;
...
if(fer_num==-1)
{
    ...
    if(MonteCarlo==1)
    {
        CString CountyIDXstr = std::to_string( CountyIDX ).c_str();
        write_MonteCarlo(sample, CountyIDXstr, cropping_system);
    }
