    ...
    for (l=BL; l<=EL; l++)
    {
     ...
     if(GN==q)//cropping land soil
     {
        if (TotalRootWT>0.0)
        {
           ...
           InputRoot_DOC[l] = ( 0.2 * TotalRootWT * fPGI) * bq * 0.3; // * 0.05;//(kg C/ha/layer)//DRco2 +
           if(N_stress[1]<1.0) InputRoot_DOC[l] *= (powf(clay[l], 0.25) / N_stress[1]);
     ...
     if(temp[l]<=0.0) 
         ...
         ch4_max = ch4_max_30 * powf(Q10CH4, (temp[l] - 0.0)/10.0); //!
     else 
...
/*	 if(InputRoot_DOC[l] > CH4_P1)
     ...
     else 
     {
       ...
       *ch4_day_oxid += CH4_O;//daily oxidation rate (kg CH4-C/ha/d)
       ...
       {
          float Fratio = 0.001 * FF * TotalRootWT * Froot * TotalVas;//0.0003	
...
void class_model::ch4_oxidation1()
{
    ...
    for(int i=1; i<=q; i++)
    {
        float OCH4 = 0.05 * (0.65 - clay[i]);//0.2
