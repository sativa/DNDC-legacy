...
void CreateDndcInputFiles(char *InputFileName)
{
	...
	//ClimateFilename.clear();
	for (i=1;i<= ClimateFileNumber;i++) 
	{
		fscanf(fp,"%d %s",&j, ClimateFilename[i]);
		
		//fgets(ClimateFilename[i],230,fp);
		//EraseFBSpace(tmp);
...
NewFarmCrop:
	...
	if(MissFlag==0) fscanf(fp,"%s %d", notes,& CropModel);
	else 
	{
...
*/
	...
	for (i=1; i<=Q; i++)
	{
		tt[i] = AnnualMeanT + (Soil_T - AnnualMeanT)/ (float)pow(i*H*100, 0.5);
...
for(int MFFC=1; MFFC<=NewTotalManureCropss; MFFC++)
{
    ...
    for (i = 1; i<=n; i++)
    {
		...
		fprintf(fp, "%d %d %f %f %f %f %f %f\n",bt,et,Dairy[i], Beef[i], Pig[i], Sheep[i],Horse[i],GrazHour[i]);
