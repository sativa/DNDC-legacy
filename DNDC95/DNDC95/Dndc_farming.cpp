    ...
    if (irr_num > 0)
    {
      for ( csl = 1; csl <= irr_num;  csl++ )
      {
        if ( jdirr[csl] == jday )
        {
            ...
            IrriMethod = irrimethod[csl]; //0: flood, 1: sprinkler, 2: drip-0cm, 3: drip-15cm
...
void class_model::fertilizing()
{
    ...
    if(FertOption==222)//auto-fertilization
    {
        ...
        for(int i1=1; i1<=5; i1++)
        {
            if(seed[i1]==1) 
            {
                FertAddN[i1] = (maxmn[i1] / ini_N_fix[i1] - ProfileNN) * ATF[3];
                if(FertAddN[i1]<0.0) FertAddN[i1] = 0.0;
                day_inFertN += FertAddN[i1];
                yr_addfer += FertAddN[i1];
                if(FertAddN[i1]>0.0)
                {
                    fert_flag = 1;
                    for (l=1; l<=2; l++) urea[l] += (FertAddN[i1] / 2.0);
                }
            }
        }
    }
...
float class_model::cutting()
{
    ...
    for ( oz = 1; oz <= cut_num; oz++ )
    {
        if ( jday==jdcut[oz])
        {
            for(int i=1; i<=1; i++)//only crop 1 is cut
            {
