...
//ttn = MData.Total_SOCSON(GN, q, rcnrvl, rcnrl, rcnrr, rcnb, rcnh, rcnm, soc, son, SIN, rcvl, rcl, rcr, CRB1, CRB2, crhl, crhr, dphum, doc, nh4, no3, no2, no, n2o, n2, nh3, urea, rcnh2, co2, sop,OrgP, LabP, ComP);
        ...
        decomp_humads(l, RFM, DDRF, till_fact, hrh, krh, &fh_co2,newh, &fh_nh4, CRB1, CRB2, dcbavai,DRCB, tilq,
            ...
            st, dphum, rcnm, q, day_O2, rcnh2, OrgP, LabP, ComP);
//ttn = MData.Total_SOCSON(GN, q, rcnrvl, rcnrl, rcnrr, rcnb, rcnh, rcnm, soc, son, SIN, rcvl, rcl, rcr, CRB1, CRB2, crhl, crhr, dphum, doc, nh4, no3, no2, no, n2o, n2, nh3, urea, rcnh2, co2, sop,OrgP, LabP, ComP);
...
*/
        ...
        if(l<=q)//&&doc[l]>0.0)
        {
            float VRmax = 5.0 * temp[l]/(40.0+temp[l]) * day_wfps[l]/(0.3+day_wfps[l]) * (1.0-clay[l])/(0.5+(1.0-clay[l]));
        
            float BAC = VRmax * doc[l]/(10.0+doc[l]) * O2[l]/(0.00015+O2[l]);//0.0003125 mol/L
...

{
    ...
    if(l>q) clay[l] = 0.01;
    float Fclay = 0.5 * powf(clay[l], -0.471);
        ...
        float *doc)
{
    ...
    //if(st[l]<12)
        Fclay = 0.1793 * powf(clay[l], -0.471) * 20.0;
    
    Fclay=max(0.0, min(1.0, Fclay));
    
    //if(st==12) Fclay *= 0.1;

    Fdepth = powf(10.0, (-20.0 * SoilDepth[l]) + 1.0);
...
#ifdef MANURE
        if(MData.m_Flash[l-q]>=1) ddf =  0.01 * powf(soil_T, 0.9) + 0.2;
        else 
        {
            if(soil_T>1.0) ddf =  0.05 * powf(soil_T, 1.0);
...
/*		ddf = 0.07 * powf(UreaAppDays, (0.05*soil_T + 0.75));
...
void class_model::nh43_equil(int l, float *temp, float *sph, float *nh4, float *nh3, int q, float *h, float *day_wfps, int flood_flag)
{
    ...
    hydrogen = powf(10.0, -sph[l]);//mol/L
    ...
    float *ice,	float *day_soil_nh3, float *nh4, int jday)
{
    ...
    if(l<=q&&nh3[l]>0.0) //field soil
    {
        ...
        float Fsd = 1.0 * (float)(q-l)/(float)q;//-3.6735 * powf((float)l, 2.0)/(float)q - 0.7551;//effect of depth
...
void class_model::NO3_leaching()//N_leaching
{
    ...
    fCLAY= 0.1 * powf(clay[1],-2.5);//-0.4907);//1.0, 0.8, 0.5
