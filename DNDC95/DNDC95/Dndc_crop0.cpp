...
void class_model::crop_N_demand(int ccwc)
{
    ...
    if(PGI[ccwc]>0.5) 
        //T_stress = (TTmax-air_temp)/(TTmax-TTopt)*powf((air_temp/TTopt), TTopt/(TTmax-TTopt));
        T_stress = (TTmax-max_temp)/(TTmax-TTopt)*powf((max_temp/TTopt), TTopt/(TTmax-TTopt));
    else
    {
    ...
    if(crop[ccwc]>0)
    {
        ...
        float FloodEffect, WTEffect=1.0, WSEffect=1.0;
            ...
            else 
            {
                ...
                WSEffect = powf(day_DWater[ccwc], 0.8);
            ...
            if(maturaty_flag[ccwc]==0&&seed[ccwc]>GermDays[ccwc])
            {
                ...
                {
                    float fPGI, dfPGI, ffPGI, GWa, GWb, GWc;
                    ...
                    else if(PGI[ccwc]<=GWc) //vegetative stage
    ...
    if(C_type[ccwc]==3&&day_N_demand[ccwc]>0.0)
    {
        ...
        else
        {
            ...
            day_N_demand[ccwc] *= powf(day_DWater[ccwc], CLAYf);
        }
    }
...
void class_model::n_uptake_by_crop(int ccwc)//for CropModel=0
{
    ...
    for ( i = 1; i <= QqQ; i++ )
    {
        UPP = day_SoilN_demand * powf(0.5, i);
        layer_up = 0.0;
...
*/
    ...
    //root_doc[ccwc] += (2.0-PGI[ccwc]) * powf(dayG_root[ccwc], 0.5) * 2.0;//Root_Wt[ccwc] * 0.1;

    if(flood_flag==1&&Root_Wt[ccwc]>0.0)
    {
        for(int l=1;l<=q;l++) R_Wt[l]=Root_Wt[ccwc]*powf(0.5, l);//kg C/ha/layer
    }
...
void class_model::growth_stage(int ccwc)
{
    ...
    if (crop[ccwc]>0&&seed[ccwc]>GermDays[ccwc])
    {
        float T_limit, LLT;
...
void class_model::root_respir(int ccwc)
{
    ...
    else
    {
        ...
        day_rco2[ccwc] = 0.025 * Root_Wt[ccwc] * AgeFactor;//Tfactor;//0.05
    }
    ...
    if (TotalRootWT>0.0&&flood_flag==0)
    {
        ...
        day_RootInput_DOC = 0.01 * TotalRootWT * fPGI * bq;//(kg C/ha/layer)
...
float class_model::crop_transpiration(int irri_close)
{
    int l, hr=1;
    ...
    for (l=1; l<=q; l++)
    {
        ...
        if(l==q)
        {
            ...
            if(DayPT0>0.0&&WaterPool>0.0001&&total_ice<=0.0) 
            {
                float add_water2=0.0;

                if(DayPT0<=WaterPool*0.5)
                {
                    add_water2 = DayPT0;
                    DayPT0 = 0.0;
                }
                ...
                //DayPT2 += add_water2;
                LayerTrans += add_water2;
                add_water2 = 0.0;
                                   
                /*if(-HighestWTdepth>=-0.5)// in case of high ground water table
                {
                    add_water2 = DayCropWaterDeficit * 0.01;//introduce imbalance due to additional water
                    DayPT2 += add_water2;
    ...
    if(DayPT0>0.0) 
    {
        DayCropWaterDeficit += DayPT0;// m water
        DayPT0 = 0.0;	
    }
    
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    if(DayCropWaterDeficit>0.0&&irri_index>0.0&&irri_close==0&&RainfedRice_flag==0) //index-irrigation
    {
        ...
        if(IrriMethod==0)//furrow irrigation
            add_water1 = 2.5 * DayCropWaterDeficit * IRRIID;	
        else if(IrriMethod==1)//sprinkler
            add_water1 = 2.0 * DayCropWaterDeficit * IRRIID;
        else if(IrriMethod==2)//dripping
            add_water1 = 1.5 * DayCropWaterDeficit * IRRIID;
        else if(IrriMethod==3)//dripping + film mulch
            add_water1 = 1.5 * DayCropWaterDeficit * IRRIID;
        else if(IrriMethod==4)//film mulch
            add_water1 = 1.0 * DayCropWaterDeficit * IRRIID;

        if(add_water1>=DayCropWaterDeficit)
        {
            DayPT2 += DayCropWaterDeficit;
            left_water = add_water1 - DayCropWaterDeficit;
            DayCropWaterDeficit = 0.0;
            DayPT0 -= DayCropWaterDeficit;
        }
        else
        {
            DayPT2 += add_water1;
            left_water = 0.0;
            DayCropWaterDeficit -= add_water1;
            DayPT0 -= add_water1;
        }

        water[1][1] += left_water; 
