...
*/
		...
		if(TotalRootWT>0.0)
		{
			root_CO2 = 0.0;//0.000001 * Day_C_growth[1] / (1.0 + PGI[1]) * TotalRootWT / (float)q;
		}
		else
		{
			root_CO2 = 0.0;
...
*/				 
			/////////Refer to Fumoto's routines//////////////////////////
			 ...
			 if (temp[l] <= 0.0) ch4_max = 0.0;
		     else if (temp[l] < 40.0) ch4_max = ch4_max_30 * powf(Q10CH4, (temp[l] - 0.0)/10.0); //!
			 else ch4_max = ch4_max_30 * Q10CH4 ;//! fixed Vmax over 40C

			 ch4_max_h2 = ch4_max;//! Vmax of CH4 production from H2
			 ch4_max_doc = ch4_max;//! Vmax of CH4 production from DOC
	...
	//// CH4 transport ////////////////////////////////////////////////////////////////////////////////////////////////
		...
		{
			//plant trnsport
			if(TotalVas>0.0&&ch4[l]>0.0&&Leaf_Wt[1]>0.0&&PGI[1]<1.0) 
			{		
				float TFT=0;
				
				if(PGI[1]<0.5) TFT = 0.4 * TotalVas * PGI[1];//Leaf_Wt[1] / 2000.0 *
				else TFT = 0.4 * TotalVas * (1.0 - PGI[1]*1.3);//Leaf_Wt[1] / 2000.0 *
