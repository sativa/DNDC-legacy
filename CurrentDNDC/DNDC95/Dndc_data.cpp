...
/*	char SYF[200];
    ...
    default_Mn = 0.01; //mol/kg soil, U.S. GEOLOGICAL SURVEY PROFESSIONAL PAPER 1270, 1984; 550 ppm.
    default_Fe = 0.46; //mol/kg soil, U.S. GEOLOGICAL SURVEY PROFESSIONAL PAPER 1270, 1984: 26000 ppm
    default_SO4 = 0.05; //mol/kg soil, U.S. GEOLOGICAL SURVEY PROFESSIONAL PAPER 1270, 1984: 1600 ppm
    //day_WT = -9.99;
    ...
    {	
        ...
        for (i = 1; i <= q; i++ )  h[i] = h[1];
        for (i = 1; i <= q; i++ ) 
        {
            int ji = (int)(SOCa/h[1]);//Depth_convert_to_layer(SOCa, h, q);//(int)(SOCa/h[1]);
            ...
            else 
            {
                float dt;
                if(SOCb>=1.0) 
                    dt = pow(SOCb, (float)( ((float)i*h[1] - SOCa)/0.1 ) );
                else
                    dt = 1.0 / ((1.0-SOCb) * (float)log((float)i-ji) + 1.0);
...
/*		wrnvl = wrcvl/rcnrvl;
    ...
    WRL = (int)(WaterRetentionDepth/h[1])-1;//Depth_convert_to_layer(WaterRetentionDepth, h, q)+1;//(int)(WaterRetentionDepth/h[1]);
...
void class_model::read_in_crop(int MODE, int GlobeData, CString FCT40)
{
    ...
    for(i=1; i<=IniCropNumbers; i++)
    {
        ...
        //Effect of CO2
        if(C_type[i]==3)
            co2_crop = (float)pow(CO2_factor, (float)0.1);
        else
            co2_crop = 1.0;
    ...
    if((crop[1]==20||crop[1]==64||crop[1]==65)&&WinterCropFlag[1]==1)
        flood_flag = 1;
}
